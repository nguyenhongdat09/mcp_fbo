...
from xml_codegraph.builder.graph_builder import GraphBuilder, build_and_save_graph, incremental_update_file, delete_node_by_path
...
def invalidate_project_cache(graph_dir: Path):
    ...
    try:
        ...
        try:
            from xml_codegraph import mcp_tools as mt
        ...
        try:
            from xml_codegraph.storage import kuzu_index as ki
            ki.close_cached_database(db_path)
        except Exception:
            pass
            
        gc.collect()
        print(f"[CodeGraph Watcher] Cleared all caches & closed DB connection for: {graph_dir_key}")
    except Exception as e:
        print(f"[CodeGraph Watcher] Warning: failed to invalidate cache: {e}")
...
def run_incremental_write(file_path: Path, action: str, controllers_root: Path, graph_dir: Path, on_node_updated=None):
    ...
    if not is_db_owner(controllers_root, graph_dir):
        print(f"[CodeGraph Watcher] user_multi_db_yn=0: Ignored write for {file_path} because project is not owner of {graph_dir}")
        return None
    ...
    for attempt in range(max_retries):
        try:
            ...
            if action == 'delete':
                ...
                if rel_path:
                    print(f"[CodeGraph Watcher] Da xoa node '{rel_path}' khoi Kuzu.")
                ...
                if node:
                    print(f"[CodeGraph Watcher] Da cap nhat thanh cong: {node.relative_path}")
                    if on_node_updated:
            ...
            if "lock" in err_msg.lower() or "io exception" in err_msg.lower():
                print(f"[CodeGraph Watcher] Attempt {attempt + 1}/{max_retries}: Lock conflict on {db_path}. Retrying in {base_delay}s... ({e})")
                time.sleep(base_delay)
                base_delay *= 2  # Exponential backoff
            else:
                print(f"[CodeGraph Watcher] Error during incremental write for {file_path}: {e}")
                # Invalidate to be safe
                invalidate_project_cache(graph_dir)
                break
                
    print(f"[CodeGraph Watcher] ERROR: Failed to write {file_path} after {max_retries} retries due to lock conflicts.")
    return None
...
class GraphUpdateHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        ...
        for path in [event.src_path, getattr(event, 'dest_path', None)]:
            ...
            if suffix not in {".xml", ".ent", ".txt", ".f"}:
                return
            if "kuzu" in str(path):
                return
        ...
        with self.write_lock:
            print(f"[CodeGraph Watcher] Phat hien su kien '{event.event_type}' tren: {event.src_path}")
            ...
            try:
                if event.event_type == 'deleted':
                    ...
                    if suffix == ".xml":
                        ...
                        if f_paired.is_file():
                            print(f"[CodeGraph Watcher] File .xml bi xoa, phuc hoi node tu file .f cung ten: {f_paired}")
                        ...
                        if xml_paired.is_file():
                            print(f"[CodeGraph Watcher] File .f bi xoa, nhung file .xml cung ten van con, khong can xoa node.")
                            return
                            
                    # Xoa thuc su khoi Kuzu DB
                    print(f"[CodeGraph Watcher] Xoa node: {event.src_path}")
                    run_incremental_write(file_path, 'delete', self.controllers_root, self.graph_dir, self.on_node_updated)
                    
                elif event.event_type == 'moved':
                    print(f"[CodeGraph Watcher] Di chuyen node tu '{event.src_path}' toi '{event.dest_path}'")
                    # 1. Xoa node cu
                    if src_in_scope:
                    ...
                    print(f"[CodeGraph Watcher] Cap nhat node: {event.src_path}")
                    run_incremental_write(Path(event.src_path), 'update', self.controllers_root, self.graph_dir, self.on_node_updated)
                    
            except Exception as e:
                print(f"[CodeGraph Watcher] Loi khi xu ly event watcher: {e}")
...
def start_watcher(controllers_root: Path, graph_dir: Path, on_node_updated=None):
    ...
    if is_unc_path:
        print(f"[CodeGraph Watcher] UNC path detected. Using PollingObserver for reliability on network shares.")
    ...
    print(f"[CodeGraph Watcher] Watcher da khoi dong tren: {scan_labels}")
    try:
        while True:
            time.sleep(1)
