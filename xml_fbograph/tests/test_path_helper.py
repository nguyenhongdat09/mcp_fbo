...
class TestDiscoverRegisteredProjects(unittest.TestCase):
    def setUp(self):
        self.temp_dir = tempfile.mkdtemp(prefix="kuzu_discover_")
        os.environ["FBOGRAPH_KUZU_BASE"] = self.temp_dir
        reset_config_caches()
    ...
    def test_discover_registered_projects(self):
        from xml_fbograph.utils.path_helper import (
            _encode_project_root,
            decode_project_root,
            discover_registered_projects,
        )

        project_root = Path(r"\\server\share\FBISP229")
        encoded = _encode_project_root(project_root)
        fbograph_dir = Path(self.temp_dir) / encoded / ".fbograph"
        fbograph_dir.mkdir(parents=True)
